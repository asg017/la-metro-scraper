...
def generate_code(report: Report):
    ...
    for idx, export in enumerate(report.exports):
        ...
        for p in export.parameters:
          py_type = py_type_from_annotated_type(p.annotated_type)
          if p.nullable:
            py_type = f"Optional[{py_type}]"
          arguments += f", {serialize_variable_name(to_snake_case(p.name))}: {py_type}"
    ...
    for class_name, columns in classes.items():
        ...
        for c in columns:
            py_type = py_type_from_decltype(c.decltype)
            if c.nullable:
                py_type = f"Optional[{py_type}]"
            lines.append(f"  {serialize_variable_name(c.name)}: {py_type}")
        lines.append('')
    # define the Db class
